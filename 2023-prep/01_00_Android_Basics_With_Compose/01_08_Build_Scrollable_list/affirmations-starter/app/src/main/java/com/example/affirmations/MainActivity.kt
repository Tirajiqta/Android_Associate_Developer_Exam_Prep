...
AffirmationList(list = Datasource().loadAffirmations())
}
...
fun AffirmationList(list: List<Affirmation>, modifier: Modifier = Modifier){
    LazyColumn(modifier = modifier){
        items(list){ current ->
            AffirmationCard(affirmation = current, modifier = Modifier.padding(8.dp))
...
fun AffirmationCard(affirmation: Affirmation, modifier: Modifier = Modifier) {
    Card(modifier = modifier) {
        Column() {
            ...
            Text(text = stringResource(id = affirmation.stringResourceId), modifier = Modifier.padding(16.dp), style = MaterialTheme.typography.headlineSmall)
        }
